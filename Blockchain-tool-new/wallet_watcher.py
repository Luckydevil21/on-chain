...
# --------------------------------------------------------------
# TRON / USDT-TRC20 - covers USDT specifically (not native TRX) via
# TronGrid, Tron's standard public API. Free, no key needed for
# light use - an optional key (TRON_API_KEY) raises the rate limit.
# --------------------------------------------------------------
...
#
# Defaults to sitting next to this script. When deploying somewhere
# with an EPHEMERAL filesystem (e.g. Render without a Persistent
# Disk attached), set TOOLKIT_DATA_DIR to a mounted disk's path
# instead, so this file survives restarts/redeploys:
...
def detect_chain(address):
    """
    Returns "ethereum", "bitcoin", "xrp", "tron", or None if the
    address doesn't look valid for any supported chain.
    """
...
# ====================================================================
# SECTION 4C: TRON / USDT-TRC20 LOGIC (via TronGrid)
# ====================================================================

def get_recent_tron_transactions(address):
    """
    PLAIN ENGLISH: Asks TronGrid for this address's most recent USDT
    (TRC-20) transfers, newest first. Covers both sent and received -
    filtering by direction happens in tron_moved_money_recently.
    """
    url = f"{TRONGRID_BASE_URL}/v1/accounts/{address}/transactions/trc20"
    params = {"contract_address": USDT_TRC20_CONTRACT, "limit": 20, "only_confirmed": "true"}
...
def tron_moved_money_recently(transactions, lookback_hours):
    """Checks whether any USDT transfer touching this address happened
    within the lookback window. Returns (tx, tx_time) or (None, None)."""
    cutoff_time = datetime.now(timezone.utc) - timedelta(hours=lookback_hours)

    for tx in transactions:
        block_timestamp_ms = tx.get("block_timestamp")
        if block_timestamp_ms is None:
            continue
        tx_time = datetime.fromtimestamp(block_timestamp_ms / 1000, tz=timezone.utc)
        if tx_time >= cutoff_time:
            return tx, tx_time
...
def print_tron_alert(wallet_address, tx, tx_time):
    """Prints a big, hard-to-miss warning banner for a Tron wallet
    that has moved USDT recently."""
    ...
    print("\n" + "🚨" * 20)
    print("🚨  ALERT: FLAGGED TRON WALLET HAS MOVED USDT  🚨")
    print("🚨" * 20)
    print(f"  Wallet Address : {wallet_address}")
    print(f"  Transaction Time (UTC): {tx_time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  Amount Moved   : {amount:.6f} USDT")
    print(f"  From           : {tx.get('from')}")
    print(f"  To             : {tx.get('to')}")
    print(f"  Tx Hash        : {tx.get('transaction_id')}")
    print(f"  View on Tronscan: https://tronscan.org/#/transaction/{tx.get('transaction_id')}")
    print("🚨" * 20 + "\n")


def check_tron_wallet(wallet_address):
    """Runs the full recent-activity check for one Tron wallet's USDT
    activity. Returns True if an alert was raised."""
    transactions = get_recent_tron_transactions(wallet_address)

    if not transactions:
        print("  No USDT transaction history found for this wallet.")
        return False

    recent_tx, tx_time = tron_moved_money_recently(transactions, LOOKBACK_HOURS)

    if recent_tx:
        print_tron_alert(wallet_address, recent_tx, tx_time)
        return True
    else:
        print("  ✅ No activity in the lookback window. Nothing to report.")
        return False
    ...
    for wallet_address in watchlist_wallets:
        ...
        if chain is None:
            print("  ⚠️  Skipping - this doesn't look like a valid Ethereum "
                  "(0x... , 42 chars), Bitcoin (1..., 3..., bc1...), XRP "
                  "(r..., 25-35 chars), or Tron (T..., 34 chars) address.")
            continue
        ...
        if chain == "ethereum":
            ...
            alert_raised = check_tron_wallet(wallet_address)
        else:
