...
def generate_evidence_pack_pdf(evidence_pack_id):
    ...
    if trace_data.get("source") == "explore_graph":
        summary_parts = [
            f"This report sets out the findings of a manually-built blockchain investigation graph "
            f"concerning {trace_data.get('wallet', '')} on {trace_data.get('chain', 'the specified chain')}. "
            f"{trace_data.get('addresses_visited', 0)} unique address(es) were examined and "
            f"{len(flagged)} transaction(s) were specifically identified and added to the graph by the "
            f"investigating analyst - not the result of an automated, exhaustive search, but of "
            f"deliberate, judgment-led selection."
        ]
    else:
        summary_parts = [
    ...
    flagged_label = "Transactions Included By The Investigating Analyst" if trace_data.get("source") == "explore_graph" else "Flagged Trail Ends (Known/Suspected Services)"
    story.extend(_path_group_flowables_with_commentary(flagged_label, flagged, styles))
    ...
    # ================================================================
    # INVESTIGATOR'S NOTES - only present for a graph built manually
    # (Explore Graph) - an automated trace has no equivalent concept.
    # Shown verbatim, exactly as the analyst wrote them - never
    # rephrased or summarized, since these are the analyst's own
    # words, not an automated finding.
    # ================================================================
    investigator_notes = trace_data.get("investigator_notes") or []
    if investigator_notes:
        story.append(heading("7a. Investigator's Notes"))
        story.append(Paragraph(
            "Free-text annotations the investigating analyst added directly to the graph during the "
            "investigation, reproduced here verbatim.",
            styles["SmallGrey"],
        ))
        story.append(spacer(2))
        for note_text in investigator_notes:
