...
def link_trace(req: LinkTraceRequest, _auth=Depends(require_read)):
    ...
    if req.direction == "backward":
        matched_paths, flagged_end_paths, addresses_visited, amount_filtered_paths = lt.trace_backward(
            req.wallet, target_lowercase_set, max_hops, req.starting_amount, req.exact_amount_only
        )
    else:
    ...
    # Sort every list of paths by when each trail actually reached its
    # endpoint (the LAST hop's real transaction time), not the order the
    # BFS happened to discover them in - without this, results jump
    # around non-chronologically (e.g. 17 Nov, then 4 Nov, then back to
    # 17 Nov) since the trace explores wallet-by-wallet, not date-by-date.
    def _last_hop_time(path):
        return path[-1]["tx_time"] if path else datetime.min.replace(tzinfo=timezone.utc)

    matched_paths.sort(key=_last_hop_time)
    flagged_end_paths.sort(key=lambda item: _last_hop_time(item[0]))
    amount_filtered_paths.sort(key=lambda item: _last_hop_time(item[0]))
