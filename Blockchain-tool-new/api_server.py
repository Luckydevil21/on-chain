...
# --------------------------------------------------------------
# Basic brute-force protection: after too many wrong keys from the
# same IP within the window, further attempts are blocked for a
# while. This is IN-MEMORY and PER-PROCESS - it resets if the server
# restarts, and does NOT share state across multiple server
# instances/workers. That's an acceptable trade-off for a single-
# instance deployment (which is what this toolkit is built for), but
# if you ever run multiple replicas behind a load balancer, this
# needs to move to a shared store (e.g. Redis) to actually work.
# --------------------------------------------------------------
_RATE_LIMIT_WINDOW_SECONDS = 300   # 5 minutes
_RATE_LIMIT_MAX_FAILURES = 10      # block further attempts after this many wrong keys in the window
_failed_auth_attempts = defaultdict(list)  # {client_ip: [timestamp, ...]}


def require_api_key(request: Request, api_key: str = Depends(_api_key_header)):
    ...
    if len(recent_failures) >= _RATE_LIMIT_MAX_FAILURES:
        raise HTTPException(
            status_code=429,
            detail=f"Too many failed authentication attempts from this address. "
                   f"Try again in a few minutes.",
        )

    if not api_key or api_key != _configured_api_key:
...
def wallet_watch(req: WalletWatchRequest, _auth=Depends(require_api_key)):
    ...
    for address in wallets:
        ...
        if chain is None:
            results.append({
                ...
                "message": "Not a recognized Ethereum, Bitcoin, or XRP address.",
            })
            continue
...
def victim_collate(req: VictimCollateRequest, _auth=Depends(require_api_key)):
    ...
    if chain is None:
        raise HTTPException(400, "Not a recognized Ethereum, Bitcoin, or XRP address.")
...
def link_trace(req: LinkTraceRequest, _auth=Depends(require_api_key)):
    """
...
def get_case_watchlist(_auth=Depends(require_api_key)):
    with _file_lock:
        return _read_case_watchlist()


@app.post("/api/case-watchlist")
def add_case_watchlist_entry(entry: CaseWatchlistEntryIn, _auth=Depends(require_api_key)):
...
def delete_case_watchlist_entry(address: str, _auth=Depends(require_api_key)):
    with _file_lock:
        entries = _read_case_watchlist()
...
def get_known_entities(_auth=Depends(require_api_key)):
    with _file_lock:
        return _read_known_entities()


@app.post("/api/known-entities")
def add_known_entity(entry: KnownEntityIn, _auth=Depends(require_api_key)):
    with _file_lock:
...
def delete_known_entity(address: str, _auth=Depends(require_api_key)):
    with _file_lock:
...
def get_deposit_map(_auth=Depends(require_api_key)):
...
def check_deposit_consolidation(req: DepositCheckRequest, _auth=Depends(require_api_key)):
    """
...
def start_address_scan(background_tasks: BackgroundTasks, _auth=Depends(require_api_key)):
    """
...
def get_latest_address_scan(_auth=Depends(require_api_key)):
    with _scan_state_lock:
