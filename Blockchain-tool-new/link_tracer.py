...
def _address_has_no_prior_history(address):
    """
    PLAIN ENGLISH: Returns True if this Bitcoin address has, at most,
    the ONE transaction that just touched it - i.e. looks like a
    freshly-generated address that's never been reused. Modern wallets
    deliberately generate a brand-new address for change specifically
    to avoid reuse, so "never used before" is a genuine, useful signal
    for change detection - on its own it's not enough (this is also
    true of any other one-time address, e.g. a rotating deposit
    address at an exchange), which is why it's only used IN
    COMBINATION with matching script type - see get_bitcoin_transaction_by_hash.
    """
    try:
...
def get_bitcoin_transaction_by_hash(tx_hash):
    ...
    # ---- Change detection - two signals, since address reuse (the OLD,
    # weaker check) rarely fires against modern wallets that deliberately
    # generate a fresh address for change every time:
    #   1. SCRIPT TYPE MATCH (free - already in the fetched data): a
    #      change output is generated by the SAME wallet software as the
    #      inputs, so it's normally the same address type (e.g. every
    #      input AND the change output are bc1q... / v0_p2wpkh).
    #   2. NO PRIOR HISTORY (one extra API call per candidate): a genuine
    #      change address is typically brand new, never used before this
    #      transaction. Capped to a modest number of checks per lookup,
    #      since a transaction with many outputs (e.g. a large CoinJoin)
    #      would otherwise trigger dozens of extra address lookups.
    # An output matching an input address DIRECTLY (the address-reuse
    # case) is still treated as confirmed change with no extra check
    # needed - that's the strongest possible signal when it happens.
    # Both tiers are labeled honestly as different confidence levels,
    # not presented as equally certain.
    ...
    input_script_types = {
        (tx_input.get("prevout") or {}).get("scriptpubkey_type")
        for tx_input in tx.get("vin", [])
        if (tx_input.get("prevout") or {}).get("scriptpubkey_type")
    }

    MAX_CHANGE_HISTORY_CHECKS = 8  # caps extra API calls per transaction lookup
    history_checks_made = 0
    ...
    for output in tx.get("vout", []):
        ...
        if address:
            ...
            if address_lower in input_addresses_lower:
                is_change = True
                change_confidence = "confirmed"  # literal address reuse - certain
            else:
                output_script_type = output.get("scriptpubkey_type")
                script_type_matches = output_script_type and output_script_type in input_script_types
                if script_type_matches and history_checks_made < MAX_CHANGE_HISTORY_CHECKS:
                    history_checks_made += 1
                    if _address_has_no_prior_history(address):
                        is_change = True
                        change_confidence = "heuristic"  # same script type + never used before - a lead, not certain
                    time.sleep(SECONDS_BETWEEN_REQUESTS)

        outputs.append({
            "address": address, "amount": amount,
            "is_change": is_change, "change_confidence": change_confidence,
        })
...
# ====================================================================
# CASE GRAPH - a persistent, growing network of every wallet and
# transaction discovered across every trace ever run against a
# specific case, rather than each trace being a disconnected, ephemeral
# result. Run a new trace next month that reaches a wallet found last
# week, and it connects into the SAME graph automatically.
#
# Deliberately stores only the RAW discovered relationship (who sent
# what to whom, when) - never entity/sanction labels, which are looked
# up FRESH every time the graph is loaded (see load_case_graph below).
# This means confirming a new Known Entity later (e.g. via the Review
# Queue) automatically and retroactively updates every existing case
# graph - nothing needs to be re-traced.
# ====================================================================

def save_case_graph_edges(case_id, hops, chain, username):
    """Persists newly-discovered hops into a case's growing graph.
    Deduplication happens at the DATABASE level (the table's own UNIQUE
    constraint) - the same transaction rediscovered by a later trace
    against the same case is silently skipped, not duplicated, so this
    is safe to call after every trace without checking first."""
    if not hops:
        return
    try:
        with auth._get_db_connection() as conn:
            with conn.cursor() as cur:
                for hop in hops:
                    cur.execute(
                        "INSERT INTO case_graph_edges "
                        "(case_id, from_address, to_address, chain, amount_label, tx_hash, "
                        "tx_time_utc, explorer_url, is_change, discovered_by) "
                        "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s) "
                        "ON CONFLICT (case_id, tx_hash, from_address, to_address) DO NOTHING;",
                        (case_id, hop["from"], hop["to"], chain, hop.get("amount_label"),
                         hop["tx_hash"], hop.get("tx_time"), hop.get("explorer_url"),
                         hop.get("is_change", False), username)
                    )
                conn.commit()
    except Exception as error:
        print(f"⚠️  Could not save case graph edges: {error}")


def load_case_graph(case_id):
    """Returns the full persisted graph for a case - every distinct
    address as a node (with entity/sanction status looked up LIVE, not
    stored), every discovered transaction as an edge."""
    try:
        with auth._get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT from_address, to_address, chain, amount_label, tx_hash, "
                    "tx_time_utc, explorer_url, is_change "
                    "FROM case_graph_edges WHERE case_id = %s ORDER BY tx_time_utc;",
                    (case_id,)
                )
                rows = cur.fetchall()
    except Exception as error:
        print(f"⚠️  Could not read case graph edges: {error}")
        return {"nodes": [], "edges": []}
    ...
    edges = []
    node_addresses = set()
    for from_addr, to_addr, chain, amount_label, tx_hash, tx_time_utc, explorer_url, is_change in rows:
        node_addresses.add(from_addr)
        node_addresses.add(to_addr)
        edges.append({
            ...
            "explorer_url": explorer_url, "is_change": is_change,
        })

    nodes = []
    for address in node_addresses:
        entity = check_known_entity(address)  # already covers sanctions first, per its own priority order
        nodes.append({
            "id": address,
            "is_case_wallet": address.lower() in case_wallet_addresses,
...
def build_seed_hops_from_bitcoin_tx(tx_hash, sender_address):
    ...
    for output in result.get("outputs", []):
        ...
        hop = {
            ...
            "explorer_url": result["explorer_url"],
            "is_change": output.get("is_change", False),
            "change_confidence": output.get("change_confidence"),
        }
        if pattern_match:
