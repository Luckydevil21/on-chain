...
# a "trace_reports" folder next to this script.
# --------------------------------------------------------------
CLEAN_OUTPUT_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "trace_reports"
)
...
#
# Populate KNOWN_ENTITIES_FILE (JSON, same folder as this script) as
# your team identifies real hot wallets/deposit addresses, e.g.:
#   [
...
# --------------------------------------------------------------
KNOWN_ENTITIES_FILE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "known_entities.json"
)
...
def find_correlated_counterpart(entity_name, reference_amount_label, reference_chain,
    ...
    """
    reference_amount, _reference_symbol = _parse_amount_and_symbol(reference_amount_label)
    if reference_amount is None:
        return []

    reference_price_usd = get_historical_price_usd(reference_chain, reference_time)
    ...
    for address, chain, _entity_type in find_group_entity_addresses(entity_name):
        ...
        for hop_info in hops:
            ...
            candidate_amount, _candidate_symbol = _parse_amount_and_symbol(hop_info["amount_label"])
            if candidate_amount is None:
                continue
            candidate_price_usd = get_historical_price_usd(chain, tx_time)
